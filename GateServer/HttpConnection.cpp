...
void HttpConnection::Start() {
	...
	beast::http::async_read(_socket, _buffer, _request, [self](beast::error_code ec, std::size_t bytes_transferred) {
		try {
			if (ec) {
				self->Start();
				return;
...
void HttpConnection::HandleReq() {
	...
	if (_request.method() == boost::beast::http::verb::post) {
		...
		if (!success) {
			_response.result(beast::http::status::not_found);
			_response.set(beast::http::field::content_type, "text/json");
			beast::ostream(_response.body()) << "url not found\r\n";
			WriteResponse();
			return;
		}
...
void HttpConnection::WriteResponse() {
	...
	beast::http::async_write(_socket, _response, [self](beast::error_code ec, std::size_t size) {
...
//当cancel触发并不会触发下面，只有当长时间没有触发上面的回调才触发定时器关闭
