...
void ContactUserList::ShowRedPoint(bool showRed)
{
    _add_friend_item->SetRedPoint(showRed);
}
...
void ContactUserList::addContactUserList()
{
    ...
    _groupitem->setFlags(Qt::NoItemFlags);
...
bool ContactUserList::eventFilter(QObject* obj, QEvent* event){
    ...
    if(obj == this->viewport() && event->type() == QEvent::Wheel){
        ...
        if(maxValue - curValue <= 0){
            bool isLoadFin = UserMgr::GetInstance()->IsLoadChatFin();
            if(isLoadFin){
                qDebug()<< "loading user finish";
                return true;
            }
