    ...
    ui(new Ui::ApplyFriend),_label_point(2,6)
{
    ...
    //ui->back_ed->setPlaceholderText(_si->_name);

    ui->lb_ed->SetMaxLen(21);
    ui->lb_ed->move(2, 2);
    ...
    connect(ui->more_lb, &ClickedOnceLabel::clicked, this, &ApplyFriend::ShowMoreLabels);
    InitTipLbs();
    //链接输入标签回车事件
    ...
    //ui->scrollArea->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff); 有什么区别
...
void ApplyFriend::ShowMoreLabels()
{
...
void ApplyFriend::addLabels(QString name)
{
...
void ApplyFriend::SetPlaceHolder(QString back_name, QString name){
    ui->name_ed->setPlaceholderText(UserMgr::GetInstance()->GetName());
    ui->lb_ed->setPlaceholderText("搜索、添加标签");
    qDebug()<<_si->_name;
    ui->back_ed->setPlaceholderText(_si->_name);
